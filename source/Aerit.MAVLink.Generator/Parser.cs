...
namespace Aerit.MAVLink.Generator
{
	public static class Parser
	{
		...
		public static (List<MessageDefinition> messages, List<EnumDefinition> enums) Run(string path, string fn, Dictionary<string, FieldType>? enumBaseTypes = null)
		{
			...
			{
				var includePath = Path.GetDirectoryName(include!.Value);
				var includeFn = Path.GetFileName(include!.Value);
			...
			elements = doc?.Root?.Element("enums")?.Elements("enum");
			if (elements is not null)
			{
				foreach (var element in elements)
				{
                    var @enum = ParseEnumDefinition(element, enumBaseTypes);
                    if (@enum.Name == "MAV_CMD")
                    {
						var cmd = enums.SingleOrDefault(o => o.Name == "MAV_CMD");
                        if (cmd is null)
                        {
                            enums.Add(@enum);
                        }
                        ...
                        {
							cmd.Entries.AddRange(@enum.Entries);
						}
					}
                    else
                    {
